 ...
 * This algorithm uses recursion to explore the maze in 4 directions (UP, DOWN, LEFT, RIGHT) with explicit backtracking.
 * It follows the same structure as BFS/DFS but uses recursive calls instead of queues/stacks.
 * The key difference is that it explicitly marks cells as UNVISITED during backtracking.
 */
public class MazeSolverRecursivoBT extends MazeSolverWorker {
	
	private boolean solutionFound;
	
	public MazeSolverRecursivoBT(Maze maze, MazeController mazeController) {
	...
	protected Boolean doInBackground() throws Exception {
		...
		return solveRecursiveWithBacktracking(start, end);
	}

	/**
	 * Método recursivo que busca la solución en 4 direcciones con backtracking explícito
	 * @param current La celda actual
	 * @param end La celda objetivo
	 * @return true si se encontró solución desde esta posición
	 */
	private boolean solveRecursiveWithBacktracking(Cell current, Cell end) throws InterruptedException {
		...
		for (Cell neighbor : validNeighbors) {
			...
			// Llamada recursiva
			if (solveRecursiveWithBacktracking(neighbor, end)) {
				current.setCurrent(false);
				return true;
			}
		}

		// BACKTRACKING: Si no se encontró solución en ninguna dirección
		// Marcar como no visitada para permitir que otros caminos puedan pasar por aquí
