 ...
 * This algorithm uses recursion to explore the maze in 2 directions (RIGHT and DOWN).
 * It follows the same structure as BFS/DFS but uses recursive calls instead of queues/stacks.
 */
public class MazeSolverRecursivo extends MazeSolverWorker {
	
	private boolean solutionFound;
	
	public MazeSolverRecursivo(Maze maze, MazeController mazeController) {
	...
	/**
	 * Método recursivo que busca la solución en 2 direcciones (derecha y abajo)
	...
	private boolean solveRecursive(Cell current, Cell end) throws InterruptedException {
		...
		// Obtener vecinos válidos en 2 direcciones (derecha y abajo)
		List<Cell> validNeighbors = getValidNeighbors2Directions(current);
	...
	/**
	 * Obtiene los vecinos válidos en 2 direcciones (derecha y abajo)
	 * @param current La celda actual
	 * @return Lista de vecinos válidos
	 */
	private List<Cell> getValidNeighbors2Directions(Cell current) {
